...
describe('ApiService', () => {
  ...
  it('should append Authorization header if token exists', () => {
    ...
    const req = httpTesting.expectOne('http://localhost:3000/api/test-endpoint');
  ...
  it('should retry up to 5 times on 5xx server errors with exponential delay', () => {
    ...
    // 1st attempt (fails)
    let req = httpTesting.expectOne('http://localhost:3000/api/test-endpoint');
    req.flush('Server Error', { status: 500, statusText: 'Internal Server Error' });
    
    // Retry 1: wait 1s
    vi.advanceTimersByTime(1000);
    req = httpTesting.expectOne('http://localhost:3000/api/test-endpoint');
    req.flush('Server Error', { status: 500, statusText: 'Internal Server Error' });
